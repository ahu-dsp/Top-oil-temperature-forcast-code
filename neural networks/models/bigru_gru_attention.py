...
class BiGRU_GRU(nn.Module):
    def __init__(self, l_hidden_size,
        ...
        self.linear2 = nn.Linear(self.g_hidden_size, output_size)
    ...
    def forward(self, input_seq):
        ...
        w_omiga = torch.randn(h_batch_size, self.g_hidden_size, 1, requires_grad=True).to(device)
        ...
        weights = self.softmax1(torch.bmm(H, w_omiga).squeeze()).unsqueeze(dim=-1).repeat(1, 1, self.g_hidden_size)

        out = torch.mul(output, weights).sum(dim=-2)
...
if __name__ == '__main__':
    batch_size = 32
    seq_len = 4
    gru_linear = BiGRU_GRU( l_hidden_size=1, l_num_layers=2, g_hidden_size=1, g_num_layers=2,output_size=1)
