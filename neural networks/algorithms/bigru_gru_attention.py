...
from models.bigru_gru_attention import BiGRU_GRU
...
if __name__ == '__main__':
    ...
    """
    batch_size = 35
    train_loader = DataLoader(train_process_data, batch_size=batch_size, shuffle=True, drop_last=False)
    test_loader = DataLoader(test_process_data, batch_size=1, shuffle=False, drop_last=False)

    # == == == == == == == == == 搭建模型 == == == == == == == == ==

    # 实例化网络模型
    lstm_linear_model = BiGRU_GRU(
        l_hidden_size=64,
        l_num_layers=1,
        g_hidden_size=64,
        g_num_layers=1,
    ...
    # gru_i = pd.DataFrame(test_predict)
    # gru_i.to_csv('jieguo.csv')
#________________________预测值写入————————————————————————————————
    # test_predict1 = pd.DataFrame(test_predict)
    # test_predict1.to_csv('12步一天滚动预测值.csv')
    ...
    plt.plot(test_number_index, test_predict, 'r--', label='prediction')
    plt.legend(loc='best')
    plt.grid(True)
    plt.show()
    plt.savefig(' ')
