...
df=pd.read_csv('', usecols=[1], engine='python')
...
'''
acf=plot_acf(train,lags = 100)
plt.title("总自相关图")
plt.show()
pacf=plot_pacf(train,method='ywm',lags = 100)
plt.title("总偏自相关图")
plt.show()       #用绘图和判断截尾或者进入平稳性的方法具有很大的主观性，所以采用aic和bic方法来辅助判断————信息准则定阶（另外还有HQIC可以使用）
trend_evaluate = sm.tsa.arma_order_select_ic(train, ic=['aic', 'bic','hqic'], trend='n', max_ar=6, max_ma=5)#为了控制计算量，在设置AR=6，MA设置为4（源程序为20,6）。 但是这样带来的坏处是可能为局部最优。
